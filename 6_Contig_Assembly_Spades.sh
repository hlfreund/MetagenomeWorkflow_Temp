...
#SBATCH --output=spades_mgm_assembly_8.2.22.stdout
#SBATCH --mail-user=hfreu002@ucr.edu
#SBATCH --mail-type=ALL
#SBATCH --job-name="Assembling MGMs with Spades 8/2/2022"
#SBATCH -p highmem
...
module load spades  # load the module on your computing cluster system (ignore this if running locally)

Path="/bigdata/aronsonlab/shared/SaltonSea/Metagenomes/SeqCenter_3.30.2023"

# Using trimmed, error-corrected, non-normalized reads for contig assembly!
for i in *_R1_clean_EC.fastq.gz;
do
    ...
    if [[ ! -d ./${SAMPLE}_assembly ]]; then
        ...
        spades.py -1 ${SAMPLE}_R1_clean_EC.fastq.gz -2 ${SAMPLE}_R2_clean_EC.fastq.gz -s ${SAMPLE}_R_unpaired_EC.fastq.gz -o ${SAMPLE}_assembly --meta -t 4 --only-assembler
